...
use Tracy\Debugger; // MDE added for debugging
class Validator extends DKIM
{
	...
	// MDE added below to allow for multiple DKIM signatures and to return reasons for failure
	public function validateWithReasons(): array {
		// Execute original validation method
		$res = $this->validate();
//		Debugger::log($res, 'validate result'); // MDE added for debugging

		$valid = true;
		foreach($res as $signature) {
			$reasons = []; // to return reasons for failure
			foreach($signature as $result) {
				if($result['status'] !== 'SUCCESS') {
					$valid = false;
					$reasons[] = $result['reason']; //  to return reasons for failure
				}
			}
		}
    ...
    public function validate(): array
    {
		...
		$signatures = array_merge($signatures, $this->getHeadersNamed('X-Google-DKIM-Signature', 'raw')); // MDE added for Google DKIM
        ...
        foreach ($signatures as $signatureIndex => $signature) {
            ...
            foreach ($dkimTags as $tagIndex => $tagContent) {
				if(strpos($tagContent, '=')) {  //MDE added to handle tags with no value which otherwise give a 'no argument 1' error
